...
def main():
    ...
    flog = open('log0413.txt', 'wr')
    cache_file = 'seg_train.pkl'
    if os.path.exists(cache_file):
        ...
        for video in video_list:
            ...
            print video_name
            video_tag, target_dx, target_dl, sample_inds, len_fg_bg = seg_sample(current_gt, seg_box, iou_h, iou_l)
            if len_fg_bg[1] == 0:
                ratio = 0
            else: 
            ...
            for i in xrange(len(seg_info)):
                ...
                seg_info[i].append(target_dx[i])
                seg_info[i].append(target_dl[i])
                if i in sample_inds:
    ...
    fout = open('select_detection_train_25fps_o75_norm_fg5_bg0to3.txt', 'w')
    for idx, line in enumerate(seg_train):
        fout.writelines('{0} {1} {2} {3} {4} {5}\n'.format(line[0], line[1], line[2], line[3], targets[idx, 1], targets[idx, 2]))
    fout.close()
...
def seg_sample(ground_truth, segment_box, iou_h, iou_l):
    ...
    return sample, target_dx, target_dl, fg_inds, len_fg_bg
