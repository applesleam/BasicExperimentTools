...
def main():
    with open('test/test_detection_videolist.txt') as f:
        list_ = f.readlines()
    with open('seg_swin_tbbox_action_0515.csv', 'rb') as f:
        file = csv.reader(f)
    ...
    wfile = open('../window_weight/weight.csv', 'rb')
    weight = [[float(i) for i in row] for row in list(csv.reader(wfile))]
    wfile.close()
    for row in seg_swin:
        row[6] = float(row[6])*weight[int(log(int(row[1])/16, 2))][cls_ind.index(row[7])-1]
    ...
    myfile = open('result/0511_6w/seg_swin_tbbox_0511_6w_weight.csv', 'wb')
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    wr.writerows(new_seg_swin)
    myfile.close()

    res_out = open('result/0511_6w/res_out_eval_tbbox_0511_6w_weight.txt', 'w')
    # submission format
...
def nms(seg_swin_per_video, pick_nms, overlap_nms, cls_ind):
    for cls in range(20):
        zipped = [(idx, [seg_swin_per_video[idx][4], seg_swin_per_video[idx][5], seg_swin_per_video[idx][6]]) for idx, row in enumerate(seg_swin_per_video) if int(cls_ind.index(row[7]))-1 ==cls]
        if len(zipped) > 0:
