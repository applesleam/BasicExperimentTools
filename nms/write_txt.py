...
import operator
def main():
    with open('result/seg_swin_tbbox_nms_once_0516_weight_sorted.csv', 'rb') as f:
        file = csv.reader(f)
    ...
    myfile = open('result/seg_swin_tbbox_nms_once_0516_weight_sorted.csv', 'wb')
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    wr.writerows(seg_swin)
    myfile.close()

    res_out = open('result/res_out_eval_nms_once_0516_weight_sorted.txt', 'w')
    # submission format
